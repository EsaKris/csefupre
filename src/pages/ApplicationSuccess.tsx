...
import { clearPaymentSession } from '../lib/applicationDraft'

export default function ApplicationSuccess() {
  usePageMeta({ title: 'Payment successful', path: '/application-success', index: false, description: 'Application payment outcome — Centre for Safety Education, FUPRE.' })
  ...
  useEffect(() => {
    if (state.kind === 'done' && !outcomePath(state.result).startsWith('/application-success')) {
      navigate(outcomePath(state.result), { replace: true, state: { result: state.result } })
    }
  }, [state, navigate])

  useEffect(() => {
    if (state.kind === 'done' && state.result.outcome === 'success') clearPaymentSession()
  }, [state])
